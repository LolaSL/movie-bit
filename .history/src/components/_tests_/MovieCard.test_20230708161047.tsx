...
describe("MovieCard", () => {
  ...
  it("renders movie card with correct data", () => {
    ...
    useSelector.mockResolvedValue({
      url: { poster: "https://api.themoviedb.org/3/posters" },
    });
  ...
  it("renders movie card without CircleRating and Genres when from search", () => {
    ...
    expect(screen.getByTestId("circle-rating-test")).not.toBeInTheDocument();
    expect(screen.getByTestId("genres-test")).not.toBeInTheDocument();
  });
});
