...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        await Promise.all([fetchApiConfig(dispatch), genresCall(dispatch)]);
      } catch (error) {
        console.log(error);
      }
    };
    ...
    fetchData();
  }, [dispatch, fetchApiConfig, genresCall]); // Empty dependency array for componentDidMount-like effect
